...
function saveCustomTheme(t: GraphTheme) {
  try {
    localStorage.setItem(LS_CUSTOM_KEY, JSON.stringify(t));
  } catch {}
}
...
function saveGroupColors(m: Record<string, string>) {
  try {
    localStorage.setItem(LS_GROUP_COLORS, JSON.stringify(m));
  } catch {}
}
...
function detectGroupKey(nodes: NodeData[]): string | null {
  ...
  for (const k of Object.keys(nodes[0])) {
    if (
      ...
      !["id", "name", "source", "target"].includes(k)
    )
      candidates.push(k);
  }
...
function buildGroupColorMap(
  ...
  const values = [
    ...new Set(nodes.map((n) => String(n[groupKey] ?? "Unknown"))),
  ];
...
function labelFromKey(key: string): string {
  const normalised =
    key === key.toUpperCase() && /[A-Z]{2,}/.test(key)
      ? key.toLowerCase()
      : key;
  return normalised
...
function formatLabel(value: string): string {
  ...
  if (
    ...
    !hasSpace &&
    !hasUnderscore &&
    value.replace(/[^A-Z]/g, "").length <= 4
  )
    return value;
  if (hasUnderscore || isAllLower || isAllUpper || isCamel)
...
function formatValue(v: unknown): string {
  ...
  if (typeof v === "number")
    return isFinite(v) ? (Number.isInteger(v) ? String(v) : v.toFixed(3)) : "—";
...
function parseGraphData(raw: unknown): GraphData | null {
  ...
  if (!Array.isArray(obj["nodes"]) || !(obj["nodes"] as unknown[]).length)
    return null;
...
const EmptyState: FC<{ theme: GraphTheme }> = ({ theme }) => (
  ...
  >
    ...
    <div style={{ fontSize: 14, fontWeight: 600, color: theme.panelText }}>
      No graph data
    </div>
    <div
      style={{
...
const InfoPanel: FC<InfoPanelProps> = ({
  ...
  const groupColor =
    groupKey && node
      ? (colorMap[String(node[groupKey] ?? "")] ?? FALLBACK_COLOR)
      : FALLBACK_COLOR;
  ...
  const stat = (label: string, value: string, color?: string) => (
    ...
    >
      ...
      <span style={{ fontSize: 14, fontWeight: 700, color: color ?? accent }}>
        {value}
      </span>
    </div>
  );
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "12px 14px" }}>
        {node ? (
          <>
            ...
            >
              <div
                style={{
                  ...
                  flexShrink: 0,
                  boxShadow: `0 0 0 3px ${groupColor}44`,
                }}
              />
              <div>
                <div
            ...
            >
              {node.avgLatency !== undefined &&
                stat("Latency", `${node.avgLatency}ms`, accent)}
              {node.avgCost !== undefined &&
                stat("Cost", `$${Number(node.avgCost).toFixed(3)}`, "#f59e0b")}
              {node.numCalls !== undefined &&
                stat(
                  "Calls",
                  Number(node.numCalls).toLocaleString(),
                  "#4ade80",
                )}
              {node.performanceScore !== undefined &&
                stat("Perf", `${node.performanceScore}%`, "#a78bfa")}
            </div>
          ...
          <>
            ...
            >
              ...
              {stat(
                "Links",
                String(totalLinks),
                theme.linkStroke !== theme.background
                  ? theme.linkStroke
                  : accent,
              )}
            </div>
...
const ThemeBar: FC<ThemeBarProps> = ({
  ...
  useEffect(() => {
    ...
    const handler = (e: MouseEvent) => {
      ...
      if (
        (t as HTMLElement)?.tagName === "INPUT" &&
  ...
  const pill = (active: boolean, accent: string): React.CSSProperties => ({
    ...
    fontSize: 12,
    fontWeight: 600,
    color: active ? accent : theme.panelText,
  ...
  const colorRow = (
    ...
    >
      <span style={{ fontSize: 12, color: theme.panelText, fontWeight: 500 }}>
        {label}
      </span>
      <input
        type="color"
    ...
    <>
      {open && (
        ...
        >
          ...
          >
            <div
              style={{ fontWeight: 700, fontSize: 13, color: theme.panelText }}
            >
              Custom Theme
            </div>
          </div>
          ...
          >
            ...
            {colorRow("Background", customTheme.background, (v) =>
              onUpdateCustom({ background: v }),
            )}
            {colorRow("Grid lines", customTheme.gridStroke, (v) =>
              onUpdateCustom({ gridStroke: v }),
            )}
            {section("Links & Labels")}
            {colorRow("Edge colour", customTheme.linkStroke, (v) =>
              onUpdateCustom({ linkStroke: v }),
            )}
            {colorRow("Label colour", customTheme.labelColor, (v) =>
              onUpdateCustom({ labelColor: v }),
            )}
            {colorRow("Node stroke", customTheme.nodeStroke, (v) =>
              onUpdateCustom({ nodeStroke: v }),
            )}
            ...
            {section("Tooltip")}
            {colorRow("Background", customTheme.tooltipBg, (v) =>
              onUpdateCustom({ tooltipBg: v }),
            )}
            {colorRow("Accent/border", customTheme.tooltipAccent, (v) =>
              onUpdateCustom({ tooltipAccent: v }),
            )}
            {colorRow("Text", customTheme.tooltipText, (v) =>
              onUpdateCustom({ tooltipText: v }),
            )}
            <button
              onClick={() => {
                onSelectCustom();
              ...
              style={{
                ...
                borderRadius: 9,
                color: customTheme.tooltipAccent,
                fontWeight: 700,
                fontSize: 13,
                cursor: "pointer",
                fontFamily: "'DM Sans','Segoe UI',sans-serif",
      ...
      >
        ...
        {PRESET_THEMES.map((t) => {
          ...
          return (
            <button
              ...
              style={pill(active, t.tooltipAccent)}
              title={t.name}
            >
              <span style={{ display: "flex", gap: 2 }}>
                {Object.values(t.modelColors)
                  .slice(0, 3)
                  .map((c, i) => (
                    <span
                      key={i}
        ...
        <button
          ...
          style={pill(isCustomActive || open, customTheme.tooltipAccent)}
        >
          ✏️ Custom
        </button>
        <div
...
export const ForceGraphComponent: FC = () => {
  ...
  useEffect(() => {
    let serial: string;
    try {
      serial = JSON.stringify(rawGraphData);
    } catch {
      return;
    }
  ...
  const [isCustomActive, setIsCustomActive] = useState(() => {
    ...
    if (!PRESET_THEMES.find((t) => t.id === id)) {
  ...
  const [savedGroupColors, setSavedGroupColors] =
    useState<Record<string, string>>(loadGroupColors);

  const graphData = useMemo(
    () => parseGraphData(stableGraphDataRef.current),
    [graphDataVersion],
  );
  ...
  const applyThemeToSvg = useCallback(() => {
    ...
    svgEl
      .selectAll<SVGLineElement, SimLink>(".link-line")
      .attr("stroke", t.linkStroke)
      .attr("stroke-opacity", t.linkOpacity);
    svgEl.selectAll<SVGGElement, SimNode>(".node-g").each(function (
      d: SimNode,
    ) {
  ...
  useEffect(() => {
    ...
    svgEl
      .selectAll(".bg-rect,.bg-grid-rect")
      .attr("width", dimensions.width)
      .attr("height", dimensions.height);
    simulationRef.current
      ?.force(
        "center",
        d3.forceCenter<SimNode>(dimensions.width / 2, dimensions.height / 2),
      )
      .alpha(0.2)
      .restart();
  }, [dimensions]);
  ...
  useEffect(() => {
    ...
    const simulation = d3
      ...
      .force(
        ...
        d3
          ...
          .radius((d) => getNodeRadius(d.numCalls) + 10),
      )
      .alphaDecay(0.04);
    ...
    nodeSel
      ...
      .on(
        "mouseenter",
        function (this: SVGCircleElement, _e: MouseEvent, d: SimNode) {
      ...
      .on(
        ...
        function (this: SVGCircleElement, _e: MouseEvent, d: SimNode) {
          d3.select<SVGCircleElement, SimNode>(this)
            ...
            .attr("stroke", themeRef.current.nodeStroke)
            .attr("stroke-width", 3);
        },
      )
      ...
      linkSel
        ...
        .attr("x2", (d) => {
          const s = d.source as SimNode,
            t2 = d.target as SimNode;
          const dx = (t2.x ?? 0) - (s.x ?? 0),
            dy = (t2.y ?? 0) - (s.y ?? 0);
  ...
  const ctrlBtn = (active = false): React.CSSProperties => ({
    ...
    justifyContent: "center",
    cursor: "pointer",
    color: active ? theme.tooltipAccent : theme.panelText,
        ...
        >
          <div
            style={{
              fontWeight: 700,
              fontSize: 13.5,
              color: getNodeColorFromMap(groupColorMap, tooltip.node, groupKey),
              marginBottom: 4,
            }}
          >
          ...
          {groupKey && (
            ...
            >
              {labelFromKey(groupKey)}:{" "}
              <strong>
                {formatLabel(String(tooltip.node[groupKey] ?? "—"))}
              </strong>
            </div>
          )}
          <div
